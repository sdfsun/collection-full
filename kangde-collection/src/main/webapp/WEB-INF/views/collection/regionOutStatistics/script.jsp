...
<script type="text/javascript" charset="utf-8">
	...
	var _search_form;
	//请求基础路径
	var baseUrl = ctx + "/collection/regionOutStatistics";
		...
		_datagrid = $('#_datagrid').datagrid({
            ...
            groupFormatter:function(value,rows){
            	...
            	for (var i=0;i<rows.length;i++){
            		row1=rows[i];
            		//alert(row1.callOutConnectCount);
            		var cont=row1.callOutConnectCount;//数量
            		count.push(cont);
            		//sum=eval(count);
            		sum += Number(row1.callOutConnectCount);
                 	return value +' - 共' + rows.length +'条数据'/* + ' 总接通次数:'+sum */;
            	}
            	return value /* +'('+ row.cno+') - ' + ' 总接通次数:0' */;
            },  
			remoteSort : true,//是否通过远程服务器对数据排序
			idField : 'id',
			columns : [ [ 
			 /* {field : 'empName', title : '风控员', width : 110},  */
			 {field : 'orgId', title : '风控方', width : 110}, 
			/*  {field : 'cno', title : '坐席号', width : 110},  */
            {field : 'customerProvince', title : '省份', width : 110,sortable:true},
           {field : 'customerCity', title : '城市', width : 110,sortable:true}, 
            {field : 'callOutCount', title : '电催总次数', width : 110,sortable:true},
            {field : 'callOutConnectCount', title : '总接通次数', width : 110,sortable:true},
            {field : 'callOutConnectDuration', title : '总通话时长', width : 110,sortable:true},
            {field : 'callOutConnectDurationAvg', title : '平均外呼通话时长', width : 120,sortable:true},
 			{field : 'callOutConnectRate', title : '外呼接通率', width : 110,sortable:true,
            ...
            {field : 'bridgeDurationDayAvg', title : '平均每日通话时长', width : 120,sortable:true},
             ] ],
