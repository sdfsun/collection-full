...
var _dialog;
//请求基础路径
var baseUrl = ctx + "/sys/assisttemplate";
$(function() {
    ...
    _datagrid = $('#_datagrid').datagrid({
        ...
        columns:[[
					 ...
					 {field:'state',title:'启用/停用',width:100,
   	...
   	if(rows.length == 0){
    		...
    		$.messager.confirm('确认提示！',
              ...
              $.each(rows, function (i, row) {
               ...
               $.post(baseUrl + '/updateForStatusCollection', {ids:ids},function(date){
               	//渲染结果
 	...
 	if(rows.length == 0){
  		...
  		$.messager.confirm('确认提示！',
            ...
            $.each(rows, function (i, row) {
             ...
             $.post(baseUrl + '/updateForStatusStop', {ids:ids},function(date){
             	//渲染结果
                 app.renderAjax(date,function(json){
                 	app.load(_datagrid);
                 },false);
        ...
        eu.showAlertMsg("只能选中一条数据进行操作！",'warning');
        	...
        	var editRoleUrl = baseUrl+"/editinputcollection";
       	    ...
       	    _dialog = app.openFormDialog(function(){
       	    	_form.submit();//提交表单
       	   	}, '编辑催记模板',600, 350,editRoleUrl+"/"+id, 'GET',function(){
       	   	formInit(num,2);
       	   	});
    	}
}
	 	...
	 	if(rows.length == 0){
	  		...
	  		$.messager.confirm('确认提示！',
	            ...
	            $.each(rows, function (i, row) {
	             ...
	             $.post(baseUrl + '/delateIds', {ids:ids},function(date){
	             	//渲染结果
	                 app.renderAjax(date,function(json){
	                 	app.load(_datagrid);
	                 },false);
...
function showDialogcollection(row) {
  ...
  //弹出对话窗口,保存按钮函数,标题,宽度,高度,请求路径,请求方式,表单加载数据函数（可以为空）
 	_dialog = app.openFormDialog(function(){
 		_form.submit();//提交表单
 	}, '新添催记模板', 600,350, inputUrl, 'GET', function(){
 		//初始化会话页面组件
 		formInit(num,2);
 		_form.form('load', row);
 	});
