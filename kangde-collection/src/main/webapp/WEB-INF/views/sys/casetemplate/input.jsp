...
<div>
	<form id="_form" class="dialog-form" method="post" novalidate>
		<table>
			<br/>
			<tr>
				<label>&nbsp;&nbsp;案件模板名称  </label> <input name="name" type="text"
					class="easyui-textbox" maxLength="100"
					data-options="required:true,missingMessage:'请输入案件模板名称',validType:'maxLength[32]'">
			</tr>
			<br/>
			<br/>
				...
				<c:forEach items="${column}" var="row" varStatus="status">
					<td><input value="${row.id}" name="list" id="${status.count}" type="checkbox" onclick="setSelectAll();"/></td>
