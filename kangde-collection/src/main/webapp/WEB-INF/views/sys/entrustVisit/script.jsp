...
var _dialog;
//请求基础路径
var baseUrl = ctx+"/sys/entrustVisit";
    ...
    _datagrid = $('#_datagrid').datagrid({
        ...
        columns:[[
			{field : 'ck', checkbox : true },
 			...
 			{field:'contactName',title:'客户联系人',width:150},
 			{field:'communicateMode',title:'沟通方式',width:150,
 				 formatter:function(value, row, index){
				 		return app.dictName('YYGL_GTFS', value);
				 		
         		}},
 			{field:'contactTime',title:'拜访日期',width:100,
 				formatter:function(value, row, index){
             		if(value){
        ...
        toolbar: [
			...
			<shiro:hasPermission name="entrustVisit:save">
             {
                text: '新增',
            ...
            <shiro:hasPermission name="entrustVisit:update">
            {
                text: '编辑',
...
function showDialog(row) {
    ...
    //弹出对话窗口,保存按钮函数,标题,宽度,高度,请求路径,请求方式,表单加载数据函数（可以为空）
   	_dialog = app.openFormDialog(function(){
   		_form.submit();//提交表单
   	}, '拜访管理', 800, 400, inputUrl, 'GET', function(){
   		//初始化会话页面组件
   		formInit();
   		if(row){
   		 row._method = "PUT";//修改表单提交方式为PUT,修改请求
   		if(row.contactTime){
       	  	row.contactTime = $.formatDate(row.contactTime,'yyyy-MM-dd');
       	  } 
   		if(row.nextFollowTime){
       	  	row.nextFollowTime = $.formatDate(row.nextFollowTime,'yyyy-MM-dd');
       	  } 
   			_form.form('load', row);
   		}
   	});
