...
eu.showMsg = function(msg) {
    $.messager.show({
        title: '<div style="color:white">提示信息！</div>',
        msg: msg,
        timeout: 3000,
...
eu.showAlertMsg = function(msgString, msgType) {
	$.messager.alert('<div style="color:white">提示信息！</div>','<div style="text-align:center;margin-top:10px;">'+msgString+'</div>', msgType);
}
...
var buttonsxxx = $.extend([], $.fn.datebox.defaults.buttons);
buttonsxxx.splice(0, 0, {
	text: '清除',
	handler: function(target){
		$(target).datebox('setValue', '');
	}
});
$.fn.datebox.defaults.buttons=buttonsxxx;
$.fn.datebox.defaults.closeText = '确定';
 ...
 * EasyUI组件加载数据通用错误提示
 * 
 * 用于datagrid/treegrid/tree/combogrid/combobox/combotree/form加载数据出错时的操作
 */
var easyuiErrorFunction = function(XMLHttpRequest) {
	$.messager.progress('close');
	app.renderAjax(XMLHttpRequest.responseText);
};
...
$.extend($.fn.tree.methods, {   
    ...
    * @param {Object} target  
    */  
   removeCheck : function(jq, target) {   
       return jq.each(function() {   
           var realTarget;   
